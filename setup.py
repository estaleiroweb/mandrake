...
def getVersion() -> str:
    ...
    try:
        ...
        ver = ''
    if ver == '':
        with open(f'{rootdir}/{projectPath}/VERSION', 'r', encoding='utf-8') as fh:
            ver =fh.read().strip()
    if ver == '':
        ver = '0.0.1'
    if '-' in ver:
...
os.chdir(rootdir)
try: shutil.rmtree(rootdir+'/dist',)
except: ...
try: shutil.rmtree(rootdir+'/build',)
except: ...
try: shutil.rmtree(rootdir+'/magic_methods.egg-info',)
except: ...
...
dictSetup = {
    ...
    'url': 'https://github.com/estaleiroweb/mandrake',
